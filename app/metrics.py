...
def calculate_voice_metrics(
    original_audio: str, generated_audio: str, sample_rate: int = 16000
) -> Dict[str, float]:
    """
    ...
    """
    logger.info(
        f"Calculating voice metrics between {original_audio} and {generated_audio}"
    )
    metrics = {}
...
def calculate_pitch_similarity(y_orig: np.ndarray, y_gen: np.ndarray, sr: int) -> float:
    ...
    try:
        ...
        f0_orig, voiced_flag_orig, _ = librosa.pyin(
            y_orig, fmin=librosa.note_to_hz("C2"), fmax=librosa.note_to_hz("C7"), sr=sr
        )
        f0_gen, voiced_flag_gen, _ = librosa.pyin(
            y_gen, fmin=librosa.note_to_hz("C2"), fmax=librosa.note_to_hz("C7"), sr=sr
        )
        ...
        f0_orig_interp = np.interp(
            np.linspace(0, 1, min_len), np.linspace(0, 1, len(f0_orig)), f0_orig
        )
        f0_gen_interp = np.interp(
            np.linspace(0, 1, min_len), np.linspace(0, 1, len(f0_gen)), f0_gen
        )
...
def calculate_spectral_similarity(
    y_orig: np.ndarray, y_gen: np.ndarray, sr: int
) -> float:
    """
...
def calculate_formant_similarity(
    y_orig: np.ndarray, y_gen: np.ndarray, sr: int
) -> float:
    """
