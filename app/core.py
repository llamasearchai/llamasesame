...
def clone_voice(
    ...
    logger.info(
        f"Generating speech with quality {quality}, temperature {temperature}, and {num_beams} beams"
    )
    with torch.no_grad():
    ...
    if compute_metrics:
        ...
        metrics = calculate_voice_metrics(
            original_audio=audio_file, generated_audio=output_path
        )
...
def batch_process(
    jobs: list, model_id: Optional[str] = None, output_dir: Optional[str] = None
) -> list:
    """
    ...
    for i, job in enumerate(jobs):
        ...
        try:
            ...
            results.append(
                {"status": "success", "output_path": output_path, "job": job}
            )
    ...
    logger.info(
        f"Batch processing completed. {len([r for r in results if r['status'] == 'success'])} successful, "
