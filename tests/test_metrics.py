...
def test_calculate_voice_metrics_integration():
    ...
    with mock.patch("llamasesame.app.metrics.librosa.load") as mock_load:
        ...
        with mock.patch("llamasesame.app.metrics.calculate_pitch_similarity") as mock_pitch, \
             mock.patch("llamasesame.app.metrics.calculate_spectral_similarity") as mock_spectral, \
             mock.patch("llamasesame.app.metrics.calculate_mfcc_similarity") as mock_mfcc, \
             mock.patch("llamasesame.app.metrics.calculate_formant_similarity") as mock_formant:
            ...
            with tempfile.NamedTemporaryFile(suffix=".wav") as temp_original, \
                 tempfile.NamedTemporaryFile(suffix=".wav") as temp_generated:
                ...
                try:
                    # Calculate metrics
                    metrics = calculate_voice_metrics(temp_original_path, temp_generated.name)
...
def test_pitch_similarity():
    ...
    y_gen = np.sin(2 * np.pi * 440 * np.arange(0, 1, 1/16000))   # Identical 440 Hz sine wave
    ...
    y_orig = np.sin(2 * np.pi * 440 * np.arange(0, 1, 1/16000))  # 440 Hz sine wave
    y_gen = np.sin(2 * np.pi * 880 * np.arange(0, 1, 1/16000))   # 880 Hz sine wave (octave higher)
...
def test_spectral_similarity():
    ...
    y_orig = np.sin(2 * np.pi * 440 * np.arange(0, 1, 1/16000))  # 440 Hz sine wave
    y_gen = 0.5 * np.sin(2 * np.pi * 440 * np.arange(0, 1, 1/16000)) + \
            0.5 * np.sin(2 * np.pi * 880 * np.arange(0, 1, 1/16000))  # Mix of 440 Hz and 880 Hz
