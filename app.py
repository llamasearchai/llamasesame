...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(os.path.expanduser("~"), "VoiceCloneOutput", "app.log"),
            mode="a",
        ),
...
def load_settings():
    ...
    settings_path = os.path.join(
        os.path.expanduser("~"), "VoiceCloneOutput", "settings.json"
    )
    try:
...
def clone_voice_ui():
    ...
    with col1:
        ...
        context_text = web.text_area(
            "Context Text (transcription of reference audio)", height=100
        )
    ...
    with col2:
        ...
        synthesis_text = web.text_area(
            "Text to Synthesize with Cloned Voice", height=100
        )

        # Model selection
        available_models = list_available_models()
        model_options = [
            {"label": details["name"], "value": model_id}
            for model_id, details in available_models.items()
        ]

        selected_model = web.select(
            "Select Model", options=model_options, default=current_model_id
        )
    ...
    if generate_button:
        with status_container:
            ...
            if not context_text:
                web.warning(
                    "Context text is empty. For best results, provide the transcription of the reference audio."
                )
            ...
            temp_dir = ensure_output_dir(
                os.path.join(os.path.expanduser("~"), "VoiceCloneOutput", "temp")
            )
            temp_file = os.path.join(
                temp_dir,
                f"reference_{get_timestamp()}_{uuid.uuid4().hex[:8]}{os.path.splitext(uploaded_file.name)[1]}",
            )
                ...
                with result_container:
                    ...
                    if settings["enable_metrics"]:
                        ...
                        with metric_cols[0]:
                            web.metric(
                                "Overall Similarity",
                                f"{metrics['overall_similarity']:.2f}",
                            )
                        with metric_cols[1]:
                            web.metric(
                                "Pitch Similarity", f"{metrics['pitch_similarity']:.2f}"
                            )
                        with metric_cols[2]:
                            web.metric(
                                "Spectral Similarity",
                                f"{metrics['spectral_similarity']:.2f}",
                            )
                        with metric_cols[3]:
                            web.metric(
                                "MFCC Similarity", f"{metrics['mfcc_similarity']:.2f}"
                            )
...
def history_ui():
    ...
    if export_button:
        export_path = os.path.join(
            os.path.expanduser("~"),
            "VoiceCloneOutput",
            f"history_export_{get_timestamp()}.json",
        )
    ...
    if search_term:
        filtered_history = [
            ...
            if search_term.lower() in entry.get("context_text", "").lower()
            or search_term.lower() in entry.get("synthesis_text", "").lower()
        ]
            ...
            with web.expander(
                ...
                if entry.get("metrics"):
                    ...
                    with metric_cols[0]:
                        web.metric(
                            "Overall Similarity",
                            f"{entry['metrics'].get('overall_similarity', 0):.2f}",
                        )
                    with metric_cols[1]:
                        web.metric(
                            "Pitch Similarity",
                            f"{entry['metrics'].get('pitch_similarity', 0):.2f}",
                        )
                    with metric_cols[2]:
                        web.metric(
                            "Spectral Similarity",
                            f"{entry['metrics'].get('spectral_similarity', 0):.2f}",
                        )
                    with metric_cols[3]:
                        web.metric(
                            "MFCC Similarity",
                            f"{entry['metrics'].get('mfcc_similarity', 0):.2f}",
                        )

                # Show generation details
                web.markdown(f"**Model:** {entry.get('model_id', 'Unknown')}")
                web.markdown(
                    f"**Quality Settings:** Quality={entry.get('quality_settings', {}).get('quality', 'Unknown')}, "
                    f"Temperature={entry.get('quality_settings', {}).get('temperature', 'Unknown')}, "
                    f"Beams={entry.get('quality_settings', {}).get('num_beams', 'Auto')}"
                )
                web.markdown(
                    f"**Duration:** {format_duration(entry.get('duration', 0))}"
                )
...
def batch_processing_ui():
    ...
    with col1:
        web.markdown(
            f"**Status:** <span style='color:{status_color};font-weight:bold'>{status}</span> - {len(batch_queue)} job(s) in queue",
    ...
    with col2:
        start_button = web.button(
            "Start Processing",
            key="start_batch",
            disabled=is_batch_processing or not batch_queue,
        )
        clear_button = web.button(
            "Clear Queue", key="clear_batch", disabled=is_batch_processing
        )
    ...
    if not batch_queue:
        ...
        for i, job in enumerate(batch_queue):
            with web.expander(
                f"Job {i+1}: {truncate_text(job.get('text', 'No text'), 50)}",
                expanded=(i == 0),
            ):
                col3, col4 = web.columns(2)
                ...
                with col3:
                    web.markdown(
                        f"**Reference Audio:** {os.path.basename(job.get('audio_file', 'Unknown'))}"
                    )
                    web.markdown(
                        f"**Context Text:** {truncate_text(job.get('context_text', 'None'), 200)}"
                    )

                with col4:
                    web.markdown(
                        f"**Synthesis Text:** {truncate_text(job.get('text', 'None'), 200)}"
                    )
                ...
                if job.get("status") == "completed":
                    web.markdown(
                        f"**Completed at:** {job.get('completion_time', 'Unknown')}"
                    )
...
def settings_ui():
    ...
    if reset_button:
        if web.confirm(
            ...
            confirm_button="Yes, reset settings",
        ):
            settings = {
                "theme": "light",
                "advanced_mode": False,
...
def add_to_history(
    ...
    if (
        ...
        and os.path.exists(reference_audio)
