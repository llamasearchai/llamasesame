...
def parse_args():
    ...
    # Clone voice command
    clone_parser = subparsers.add_parser("clone", help="Clone a voice from a reference audio")
    clone_parser.add_argument("--audio", "-a", required=True, help="Path to reference audio file")
    clone_parser.add_argument("--context", "-c", required=False, help="Context text (transcription of reference audio)")
    clone_parser.add_argument("--text", "-t", required=True, help="Text to synthesize with cloned voice")
    clone_parser.add_argument("--model", "-m", default="sesame/csm-1b", help="Model ID to use")
    clone_parser.add_argument("--quality", "-q", type=int, default=5, choices=range(1, 11), help="Quality level (1-10)")
    clone_parser.add_argument("--temperature", type=float, default=0.8, help="Generation temperature")
    clone_parser.add_argument("--beams", type=int, help="Number of beams for generation")
    clone_parser.add_argument("--seed", type=int, help="Random seed for reproducibility")
    clone_parser.add_argument("--output", "-o", help="Output filename")
    clone_parser.add_argument("--output-dir", help="Output directory")
    
    # Batch processing command
    batch_parser = subparsers.add_parser("batch", help="Process a batch of voice cloning jobs")
    batch_parser.add_argument("--file", "-f", required=True, help="JSON file with batch jobs")
    batch_parser.add_argument("--model", "-m", help="Default model ID to use (overrides job-specific model)")
    batch_parser.add_argument("--output-dir", help="Output directory")
    
    # List models command
    list_parser = subparsers.add_parser("list-models", help="List available voice cloning models")
...
def batch_from_file(file_path: str, model_id: Optional[str] = None, output_dir: Optional[str] = None):
